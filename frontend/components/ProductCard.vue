<template>
  <div class="border p-4 rounded-lg shadow-md flex flex-col justify-between">
    <div>
      ...
      <p v-if="product.stock < 10" class="text-red-500 text-xs mt-1">Stok Tersisa: {{ product.stock }} </p>
      <p v-if="product.stock >= 10" class="text-green-500 text-xs mt-1">Stok Tersisa: {{ product.stock }} </p>
    </div>
    <button
