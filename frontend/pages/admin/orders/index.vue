      ...
      <table v-else class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="order in filteredOrders" :key="order.id">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ formatDateDisplay(order.created_at) }}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">Rp {{ order.total_amount ? parseFloat(order.total_amount).toLocaleString('id-ID') : '0' }}</td>
    ...
    <div v-if="showOrderModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div class="bg-white p-6 rounded-lg shadow-xl w-full max-w-lg overflow-y-auto max-h-[90vh]">
        ...
        <div v-if="!isEditing && currentOrder" class="mb-4">
          <p><strong>Tanggal:</strong> {{ formatDateDisplay(currentOrder.created_at) }}</p>
          <p><strong>Total:</strong> Rp {{ currentOrder.total_amount ? parseFloat(currentOrder.total_amount).toLocaleString('id-ID') : '0' }}</p>
          ...
          <ul class="space-y-3">
            <li v-for="item in (currentOrder.items || [])" :key="item.id" class="flex items-center gap-3 p-2 border rounded-md bg-gray-50">
              ...
              <div class="flex-grow">
                ...
                <p class="text-sm text-gray-600">Qty: {{ item.quantity || 0 }} x Rp {{ item.price_at_purchase ? parseFloat(item.price_at_purchase).toLocaleString('id-ID') : '0' }}</p>
                <p class="text-sm font-semibold text-gray-700">Subtotal: Rp {{ (item.quantity * parseFloat(item.price_at_purchase)) ? (item.quantity * parseFloat(item.price_at_purchase)).toLocaleString('id-ID') : '0' }}</p>
              </div>
            </li>
          </ul>
        </div>
...
const productImageUrl = (relativePath) => {
    if (relativePath) {
        // Asumsi base URL media ada di runtimeConfig.public.mediaBaseUrl
        return `${runtimeConfig.public.mediaBaseUrl}${relativePath}`;
    }
    return '/no-image.png'; // Atau URL gambar placeholder default jika tidak ada gambar
};
...
const filteredOrders = computed(() => {
  ...
  // 1. Filter Pencarian (searchQuery)
  ...
  // 2. Filter Tanggal (filterDate)
  if (filterDate.value) {
    orders = orders.filter(order => {
      // Pastikan order.created_at ada
      if (!order.created_at) return false;
      
      // Ambil tanggal dari created_at dan konversi ke YYYY-MM-DD
      // Gunakan formatDateToYMD (fungsi baru di bawah) untuk memastikan format konsisten
      const orderDateFormatted = formatDateToYMD(order.created_at);
      
      // console.log(`Comparing: ${orderDateFormatted} === ${filterDate.value}`); // Debugging
      return orderDateFormatted === filterDate.value;
    });
  }

  // 3. Filter Status (filterStatus)
  ...
  return orders.sort((a, b) => {
    ...
    const dateA = a.created_at ? new Date(a.created_at) : new Date(0); // Gunakan new Date(0) sebagai fallback
    const dateB = b.created_at ? new Date(b.created_at) : new Date(0);
    
    // Periksa validitas setelah konversi
    if (isNaN(dateA.getTime())) return 1; // Pindahkan order A ke bawah jika tidak valid
    if (isNaN(dateB.getTime())) return -1; // Pindahkan order B ke bawah jika tidak valid

    return dateB.getTime() - dateA.getTime(); // Urutkan dari terbaru ke terlama
  });
});

// Fungsi untuk memformat tanggal untuk tampilan (contoh: 18 Juli 2025)
const formatDateDisplay = (dateString) => {
  if (!dateString) return ''; 
  try {
    const date = new Date(dateString);
    if (isNaN(date.getTime())) {
      console.warn("Invalid date string provided to formatDateDisplay:", dateString);
      return dateString; 
    }
...
// Fungsi pembantu untuk memformat tanggal ke YYYY-MM-DD (khusus untuk perbandingan filter)
const formatDateToYMD = (dateString) => {
  ...
  try {
    ...
    if (isNaN(date.getTime())) {
      ...
      return ''; // Kembalikan string kosong jika tidak valid untuk filter
    }
